...
| `Int1` | 布尔值 | `bool` | `bool` |
| `Int32` | 32位整数 | `int` | `i32` |
| `Int` | 通用整数类型，兼容 Int1/Int32/指针 | `size_t` | `int` |
| `Float32` | 单精度浮点数 | `float` | `f32` |
| `Float64` | 双精度浮点数 | `double` | `f64` |
| `Float` | 通用浮点类型，兼容 Float32/Float64 | `double` | `float` |

通用类型 `Int` 和 `Float` 的子类型关系：
- `Int <: Int32`, `Int32 <: Int`（双向兼容）
- `Int <: Int1`, `Int1 <: Int`（双向兼容）
- `Int <: Reference`, `Reference <: Int`（整数与指针双向兼容）
- `Float <: Float32`, `Float32 <: Float`（双向兼容）
- `Float <: Float64`, `Float64 <: Float`（双向兼容）
...
1. `from is ⊥`: `true`
2. `to is ⊤`: `true`
3. `{from, to} is primitive_type`: 类型相同则是子类型，另外 Int/Float 通用类型与对应具体类型双向兼容
...
- 指令有四种，分别是一元指令、二元指令、函数调用指令和 Phi 指令。其中地址是 `Value`，结果是 `optional<LeftValue>`（结果可以为空，例如 `STORE` 指令）
...
- 临时值的格式是 `%{id}`
- 变量定义：`const`/`let`，函数定义：`fn`，一般语句：`{result}: {type} = {expression}` 或 `{expression}`（当 result 为空时）
