...
其中 `100-103` 为额外编写的边界用例：`100_many_args` 测试多参数（23 个，7 个走栈）混合 int/float/double/bool 类型的 ABI；`101_large_array_init` 测试大数组尾部零元素优化；`102_large_stack` 测试栈帧偏移超出 12 位立即数范围的大栈场景；`103_stack_spill` 测试 32 个局部变量强制寄存器溢出的高寄存器压力场景。
...
`fib_subseq` 因内联和窥孔优化大幅领先 gcc；`matrix` 和 `tim_sort` 与 gcc 持平或略优。
