...
SSA 销毁（`DestructSSA`）阶段依赖数据流分析，而数据流分析要求控制流图连通。如果 SSA 优化后遗留了不可达的基本块，数据流分析的迭代求解会出错。

我们将 `DeadBlockElimination` 作为 `DestructSSA` 的第一个 pass（在 `SplitCriticalEdge` 和 `ReplacePhi` 之前执行），确保 SSA 销毁前 CFG 总是连通的。
