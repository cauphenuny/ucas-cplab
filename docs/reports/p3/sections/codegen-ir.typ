...
我们实现的寄存器分配大体上分为4个pass：
  ...
  构建冲突图、图着色 `(Colorize)`
    ...
    在扫描指令收集变量时，对每个出现的变量调用 `ensure()` 确保其存在于干涉图中，即使度数为零。否则零度节点（从未与任何变量冲突的节点）不会出现在图中，后续着色阶段会报错。同时增加了 `need_register()` 过滤器，跳过 `reference` 和 `comptime` 属性的变量——它们存放在栈上或编译期已知，不需要寄存器。
