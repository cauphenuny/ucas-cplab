...
template <typename T>
struct is_serializable : std::bool_constant<string_constructible<T>::value ||
...
template <typename T> constexpr bool is_serializable_v = is_serializable<T>::value;

template <typename T> auto serialize(const std::unique_ptr<T>& ptr) -> std::string {
