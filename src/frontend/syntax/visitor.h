...
public:
    ...
    std::any visitConstInitVal(CACTParser::ConstInitValContext *ctx) override {
        ...
        if (ctx->constExp()) {
            ...
            for (auto *child : ctx->constInitVal()) {
                vals.push_back(std::make_unique<ast::ConstInitVal>(take<ast::ConstInitVal>(visit(child))));
            }
    ...
    std::any visitNumber(CACTParser::NumberContext *ctx) override {
        if (ctx->INT_LITERAL()) return wrap(ast::ConstExp(std::stoi(ctx->INT_LITERAL()->getText(), nullptr, 0)));
        if (ctx->FLOAT_LITERAL()) return wrap(ast::ConstExp(std::stof(ctx->FLOAT_LITERAL()->getText())));
        if (ctx->DOUBLE_LITERAL()) return wrap(ast::ConstExp(std::stod(ctx->DOUBLE_LITERAL()->getText())));
        return {};
    }
    ...
    std::any visitStmt(CACTParser::StmtContext *ctx) override {
        if (ctx->lVal()) {
            return wrap(ast::AssignStmt{
                .var = take<ast::LValExp>(visit(ctx->lVal())),
    ...
    std::any visitEqExp(CACTParser::EqExpContext *ctx) override {
        if (ctx->eqExp()) {
            ast::BinaryExp bin;
            bin.op = (ctx->getText().find("==") != std::string::npos) ? ast::BinaryOp::EQ : ast::BinaryOp::NEQ;
    ...
    std::any visitRelExp(CACTParser::RelExpContext *ctx) override {
        if (ctx->relExp()) {
            ...
            if (text == "<") bin.op = ast::BinaryOp::LT;
            else if (text == ">") bin.op = ast::BinaryOp::GT;
            else if (text == "<=") bin.op = ast::BinaryOp::LEQ;
            else bin.op = ast::BinaryOp::GEQ;
    ...
    std::any visitMulExp(CACTParser::MulExpContext *ctx) override {
        if (ctx->mulExp()) {
            ast::BinaryExp bin;
            auto text = ctx->children[1]->getText();
            if (text == "*") bin.op = ast::BinaryOp::MUL;
            else if (text == "/") bin.op = ast::BinaryOp::DIV;
    ...
    std::any visitUnaryExp(CACTParser::UnaryExpContext *ctx) override {
        ...
        if (text == "+") unary.op = ast::UnaryOp::PLUS;
        else if (text == "-") unary.op = ast::UnaryOp::MINUS;
        else unary.op = ast::UnaryOp::NOT;
    ...
    std::any visitPrimaryExp(CACTParser::PrimaryExpContext *ctx) override {
        ...
        if (ctx->lVal()) return wrap(ast::Exp(ast::PrimaryExp(take<ast::LValExp>(visit(ctx->lVal())))));
        if (ctx->number()) return wrap(ast::Exp(ast::PrimaryExp(take<ast::ConstExp>(visit(ctx->number())))));
        if (ctx->boolNumber()) return wrap(ast::Exp(ast::PrimaryExp(take<ast::ConstExp>(visit(ctx->boolNumber())))));
        return {};
    }
