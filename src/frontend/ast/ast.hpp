...
struct CompUnit : public mixin::Locatable {
    ...
    CompUnit(Location loc, std::vector<Item> items) : items(std::move(items)) {
        this->loc = loc;
    }
    CompUnit(CompUnit&&) = delete;  // NOTE: AST is not movable because some nodes's address will be
                                    // used as keys in semantic analysis
};
