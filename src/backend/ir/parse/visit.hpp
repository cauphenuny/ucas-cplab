...
public:
    IRConstructVisitor() {
        ...
        auto addPrimitiveBuiltin = [&](const std::string& name, adt::TypeBox param_type) {
            auto params = adt::Product();
            params.append(std::move(param_type));
            auto type = adt::Func(std::move(params), adt::construct<void>()).toBoxed();
            program_.addBuiltinFunc(std::make_unique<ir::BuiltinFunc>(name, std::move(type)));
        };
        addPrimitiveBuiltin("print_int", adt::construct<int>());
        addPrimitiveBuiltin("print_float", adt::construct<float>());
        addPrimitiveBuiltin("print_bool", adt::construct<bool>());
        addPrimitiveBuiltin("print_double", adt::construct<double>());
    }
    ...
    std::any visitFuncDecl(IRParser::FuncDeclContext* ctx) override {
        ...
        auto func = std::make_unique<ir::Func>(std::move(ret_type), name, std::move(params));
        ...
        program_.addFunc(std::move(func));
...
private:
    ...
    auto resolveDef(IRParser::VarContext* ctx, const adt::TypeBox& type) -> ir::LeftValue {
        if (ctx->temp()) {
            ...
            auto name = ctx->ID()->getText();
            // ID must be pre-defined in localDecl or paramList
            if (local_symbol_map_.count(name)) {
                auto alloc = local_symbol_map_.at(name);
                return ir::NamedValue{.type = alloc->type, .def = alloc};
            }
    ...
    auto resolveFunc(const std::string& name) -> std::optional<std::pair<adt::TypeBox, ir::NameDef>> {
        ...
        // Try to find in current program
        try {
